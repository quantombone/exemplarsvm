...
for q = 1:length(models)
  if (size(hn.xs{q},2) >= mining_params.MAX_WINDOWS_BEFORE_SVM) || ...
    ...
    models{q} = update_the_model(models,hn,q,mining_params);
  end
end

return;

fprintf(1,' ##Plotting for iteration %d\n',iteration);
figure(1)
clf

subplot(2,2,1)
I = get_exemplar_icon({m},1);
imagesc(I)
axis image
axis off
cls = '';
...
axis image
axis off
template_diff = norm(m.model.w(:)-lastw(:));
...
subplot(2,2,4)
Isv = get_sv_stack(m.model.svids(1:min(length(m.model.svids),25)),bg);
imagesc(Isv)
axis image
axis off
title('Top 25 -SVs');
...
function m = update_the_model(models,hn,index,mining_params)
m = models{index};
