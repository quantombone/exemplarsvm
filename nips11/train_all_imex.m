...
files = dir([initial_directory '*imex.mat']);
...
for i = 1:length(ordering)
  ...
  models = models{1}.ms;
  for aaa = 1:length(models)
    models{aaa}.models_name = 'nips11';
  end
  filer2fill = sprintf('%s/%%s.%s.%05d.mat',final_directory, ...
                        models{1}.curid,ordering(i));
  ...
  for q = 1:length(models)
    bg = intersect(bg,get_pascal_bg('train',['-' models{q}.cls]));
  end
  fprintf(1,'final bg has len %d\n',length(bg));
  
  %%choose only negative that are negative to all images
