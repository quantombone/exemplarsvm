...
for qqq = 1:5
  ...
  %R = R / max(R(:));
  %R(maxos<.1) = -1;
  %R(negatives) = min(-1,R(negatives));
  
  respos = m.model.w(:)'*m.model.nsv(:,[pos negatives]) - m.model.b;
  [aa,bb] = sort(respos,'descend');
  [alpha,beta] = sort(bb);
  weights = (1./beta).^.3;
  weights(ismember(1:length(beta),pos) & (beta >=5)) = 0;
  ...
  C = diag(weights);
  %R((1:length(pos))) = weights(1:length(pos));
