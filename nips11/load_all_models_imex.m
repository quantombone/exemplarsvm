function [models] = load_all_models_imex(cls, DET_TYPE, FINAL_PREFIX)
...
m = cell(1,82);
for i = 1:82%;length(files)
  fprintf(1,'.');
  filer = sprintf('%s/100.*.%05d.mat',results_directory,i);
  files = dir(filer);

  m{i} = load([results_directory files(1).name]);
  m{i} = m{i}.models;
end
models = [m{:}];
