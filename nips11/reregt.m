...
for indexi = 1:length(models)
  ...
  for bbb = 1:NITER
    ...
    drawnow
        ...
        for q = 1:10
      curid = m.model.svids{q}.curid;
      I = convert_to_I(bg{curid});
      mp = get_default_mining_params;
      mp.SAVE_SVS = 1;
      rs = localizemeHOG(I,{m},mp);
      idgrids = rs.id_grid{1};
      for z = 1:length(idgrids)
        idgrids{z}.curid = curid;
      end
      if length(idgrids) > 0
        savem.model.svids = cat(2,savem.model.svids,idgrids);
        svs = cat(2,rs.support_grid{1});
        svs = cat(2,svs{:});

        savem.model.nsv = cat(2,savem.model.nsv,svs);
      end
    end
