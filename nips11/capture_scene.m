function [Is,xs,scores,bbs] = capture_scene(NITER,ws,bs,TOPK,catnames,M)
...
for i = 1:NITER
  ...
  if exist('bs','var')
    ...
    curI = min(1.0,max(0.0,imresize(I,[200 200])));
    curx = features(curI,20);
    x = curx(:);
    ws = ws ./repmat(sqrt(sum(ws.^2)),size(ws,1),1);
    r = ws'*x-bs;
