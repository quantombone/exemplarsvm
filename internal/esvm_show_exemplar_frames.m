...
for i = 1:length(pinds)
  ...
  for i = 1:N
    ...
    colors = [1 0 0; 0 0 1];
    cim = colors(onimage(:),:);
    cim = reshape(cim,[size(m.mask,1) size(m.mask,2) 3]);
    imagesc(cim);
    fullimbox = [0 0 size(cim,2) size(cim,1)]+.5;
    xform = find_xform(m.bb(1,1:4), fullimbox);
    ...
    title(sprintf('Template:[%d x %d] \ncuros=%.2f Mask: [%d x %d]',...
                  m.hg_size(1), m.hg_size(2),...
                  curos,range(u)+1,range(v)+1));
    
    subplot(N,3,o+3)
    hogim = HOGpicture(repmat(m.mask,[1 1 size(m.w,3)]).* ...
                       m.w);
    imagesc(hogim)
    axis image
    axis off
