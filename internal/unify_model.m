...
% fat model which stores latent positive update features for each
% location

if isfield(model.models{1},'extra_models')
...
% bb = cat(1,bb{:});
resc = get_canonical_bb_old(model);
...
gtbb = cellfun2(@(x)x.gt_box,model.models);
gtbb = cat(1,gtbb{:});
...
model2.models{1}.gtbb = gtbb;


model2.models{1}.resc = get_canonical_bb_old(model);%(model2.models{1}.bb,gtbb,hg_size);
